...
export const rotateBase64 = (base64: string, degree: number): Promise<string> => {
...
// Background Color #f6f6f6 chagne Function in 'body tag'
export const backgroundColorChange = (status: 'on' | 'off') => {
  if (status === 'on') document.body.classList.add('back-gray');
  if (status === 'off') document.body.classList.remove('back-gray');
};
